 ...
 */
#define tskSTACK_FILL_BYTE	( 0x5aU )
...
void *test_get_currentTcb_ptr(void)
{
	return pxCurrentTCB;
}
...
void vTaskStackDump(TaskHandle_t xTaskToQuery)
{
    TCB_t *pxTCB = (TCB_t *)xTaskToQuery;

    if (NULL == pxTCB) {
        pxTCB = pxCurrentTCB;
    }
    
    if (0x00 == pxTCB->pcTaskName[0]) {
        bk_printf("task name : %s\r\n", "(null)");
    } else {
        bk_printf("task name : %s\r\n", pxTCB->pcTaskName);
    }
    
    bk_printf("top stack: %p, stack: %p, uxSizeOfStack: 0x%x\r\n", pxTCB->pxTopOfStack, pxTCB->pxStack, pxTCB->uxSizeOfStack);
    //bk_printf("Backtrace run by addr2line -e OTA2.axf -f ");
    bk_printf("Backtrace: ");

    volatile StackType_t *tmpStack = pxTCB->pxTopOfStack;
    while (tmpStack < pxTCB->pxStack + pxTCB->uxSizeOfStack) {
        if (((*tmpStack >= 0x00010000) && (*tmpStack <= 2*1024*1024)) 
            || ((*tmpStack >= 0X003FEE00) && (*tmpStack <= 0X003FEE00 + 4608)))
        {
            bk_printf("%x ", *tmpStack);
        }
        tmpStack++;
    }
    bk_printf("\r\n\r\n");

    tmpStack = pxTCB->pxTopOfStack;
    while (tmpStack < pxTCB->pxStack + pxTCB->uxSizeOfStack) {
        if (((*tmpStack >= 0x00010000) && (*tmpStack <= 2*1024*1024)) 
            || ((*tmpStack >= 0X003FEE00) && (*tmpStack <= 0X003FEE00 + 4608)))
        {
            if (*tmpStack & 0x00000001) {
                bk_printf("%x ", *tmpStack-1);
            } else {
                bk_printf("%x ", *tmpStack);
            }
        }
