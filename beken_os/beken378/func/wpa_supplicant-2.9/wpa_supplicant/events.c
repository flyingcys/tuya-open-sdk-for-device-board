...
int get_security_type_from_ie(u8 *ie_start, int len, u16 caps)
{
	...
	if (rsn_ie) {
		if (wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &ie) == 0) {
			if ((ie.key_mgmt & (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_PSK))
					== (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_PSK)) {
				return SECURITY_TYPE_WPA3_WPA2_MIXED;;
			} else if ((ie.key_mgmt & WPA_KEY_MGMT_SAE) == WPA_KEY_MGMT_SAE) {
				return SECURITY_TYPE_WPA3_SAE;;
			} else {
	...
	if (wpa_ie) {
		if (wpa_parse_wpa_ie(wpa_ie, 2 + wpa_ie[1], &ie) == 0) {
			if(ie.pairwise_cipher == WPA_CIPHER_CCMP)
					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	while ((ssid->proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN)) && rsn_ie) {
        ...
        if (wpa_config_set_wpa(ssid, &ie)) {
            if (debug_print) {
                wpa_dbg(wpa_s, MSG_DEBUG, "   skip NO WPA - PSK failed");
            }
            break;
        }
				     ...
				     int only_first_ssid, int debug_print)
{
	...
	const u8 *ie;
	struct wpa_ssid *ssid;
	int osen __maybe_unused, rsn_osen __maybe_unused = 0;
#ifdef CONFIG_MBO
	...
	for (ssid = group; ssid; ssid = only_first_ssid ? NULL : ssid->pnext) {
		...
		} else if (0 == ssid->ssid_len) {
			if (NULL != ssid->ssid) {
				os_free(ssid->ssid);
			}
			ssid->ssid = dup_binstr(bss->ssid, bss->ssid_len);
			if (NULL != ssid->ssid) {
		...
		if (!wpa_supplicant_match_privacy(bss, ssid, wpa)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					      ...
					      int own_request, int update_only)
{
	...
	if (scan_res == NULL) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Failed to get scan results - try "
			"scanning again, , retry cnt = %d.", g_sta_param_ptr->retry_cnt);
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ &&
		...
		manual_scan_use_id = wpa_s->manual_scan_use_id;
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		if(supplicant_main_is_exit()) {
			// wpa_global_ptr is null, may be del vif
			return 2;
		}
