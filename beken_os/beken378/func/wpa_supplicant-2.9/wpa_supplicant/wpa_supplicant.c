			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		int fils_hlp_sent __maybe_unused = 0;
		...
		if (state == WPA_DISCONNECTED)		// added by tiancj
...
void wpa_supplicant_associate(struct wpa_supplicant *wpa_s,
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	struct wpa_connect_work *cwork __maybe_unused;
	int rand_style;
...
#if !defined(CONFIG_SME) || defined(CONFIG_FULL_SUPPLICANT)
...
{
	int ieee80211_mode __maybe_unused = wpas_mode_to_ieee80211_mode(ssid->mode);
	enum hostapd_hw_mode hw_mode;
	struct hostapd_hw_modes *mode = NULL;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (bss && bss->ie_len  && get_ie((u8 *)(bss + 1), bss->ie_len, WLAN_EID_SUPP_RATES)) {
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid) {
		...
		wpa_s->connect_without_scan =
			(ssid->mode == WPAS_MODE_MESH /*|| ssid->fast_connect*/) ? ssid : NULL;
...
int radio_add_work(struct wpa_supplicant *wpa_s, unsigned int freq,
		   ...
		   void *ctx)
{
	struct wpa_radio *radio __maybe_unused = wpa_s->radio;
	struct wpa_radio_work *work;
	int was_empty;
...
int wpas_network_disabled(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt) && !ssid->psk_set &&
	    (!ssid->passphrase || os_strlen(ssid->passphrase) < 8 || ssid->ssid_len != 0) && !ssid->ext_psk &&
