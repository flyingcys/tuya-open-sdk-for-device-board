					 ...
					 struct wpa_bss *bss)
{
	...
	while ((ssid->proto & WPA_PROTO_RSN) && rsn_ie) {
		...
		if (wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &ie)) {
			os_null_printf("   skip RSN IE - parse "
				"failed\r\n");
			break;
		}
		...
		{
			os_printf("   selected based on TSN "
				"in RSN IE\r\n");
			return 1;
		}
...
int wpa_supplicant_connect(struct wpa_supplicant *wpa_s,
			   ...
			   struct wpa_ssid *ssid)
{
	os_null_printf("wpa_supplicant_connect\r\n");
					      ...
					      int own_request)
{
	...
	if (scan_res == NULL) {
		...
		os_printf("Failed to get scan results, retry cnt=%d\r\n", 
	...
	if (wpa_s->disconnected) {
		wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
		os_printf("Failed to get scan results - disconnect\r\n");
		ret = -3;
		goto scan_work_done;
	}
					      ...
					      int new_scan, int own_request)
{
	...
	if (time_to_reenable > 0) {
		os_null_printf("Postpone network selection by %d seconds since all networks are disabled\r\n",
			time_to_reenable);
	...
	if(supplicant_main_is_exit())
	{
		...
		if (ssid) {
			os_null_printf("Setup a new network\r\n");
					     ...
					     union wpa_event_data *data)
{
	...
	if (res == 2) {
		 ...
		 * wpa_s after this.
		 */
		os_null_printf("Interface may removed\r\n"); 
		return 1;
	}
	if (res != 0) {
		/*
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->conf->ap_scan == 1 &&
	    (0 != (wpa_s->drv_flags & WPA_DRIVER_FLAGS_BSS_SELECTION))) {
				 ...
				 union wpa_event_data *data)
{
    ...
    {
    struct wpa_bss *bss;
    bss = wpa_bss_get_bssid(wpa_s, data->ft_ies.target_ap);
    if (bss)
    {
        wpa_s->sme.freq = bss->freq;
        wpa_s->sme.ssid_len = bss->ssid_len;
        if (bss->ssid_len > 0)
