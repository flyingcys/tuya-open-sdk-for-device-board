...
void ap_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	os_null_printf("ap_handle_timer:%d,%ds\r\n", fclk_get_tick(), fclk_get_second());	
	...
	if ((sta->flags & WLAN_STA_ASSOC) &&
		...
		if (inactive_sec == -1) {
			os_null_printf("Check inactivity: Could not "
			...
			os_null_printf("Station " MACSTR " has lost its driver entry",
				MAC2STR(sta->addr));
