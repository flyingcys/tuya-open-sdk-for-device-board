...
void sctrl_sta_ps_init(void)
{
    ...
    os_null_printf("sctrl_sta_ps_init\r\n");
    #endif
...
void sctrl_init(void)
{
    ...
    /* huaming20220625: clock source change policy: "high->low: mux,div; low->high: div,mux" */
	...
	if ((DEVICE_ID_BK7231N_P & DEVICE_ID_MASK) == (sctrl_ctrl(CMD_GET_DEVICE_ID, NULL) & DEVICE_ID_MASK))
	{
		param = 0x580120E2;//wangjian20210422<28:23>=30 as default for BK7231P, qunshan20211109<17:16>=1 for BK7231P
	}
	else
	{
...
UINT32 sctrl_mcu_wakeup(void)
{
    ...
    if(sctrl_mcu_ps_info.hw_sleep == 1)
    {
        ...
        #endif
        #if(CFG_SOC_NAME == SOC_BK7231N)
        UINT32 reg = REG_READ(ICU_PERI_CLK_PWD);
        reg &= ~(PWD_ARM_WATCHDOG_CLK_BIT);
        REG_WRITE(ICU_PERI_CLK_PWD, reg);
        #endif
        wkup_type = 1;
    ...
    if ((deep_param->wake_up_way & PS_DEEP_WAKEUP_GPIO))
    {
        for (i = 0; i < BITS_INT; i++)
        {
            ...
            if (deep_param->gpio_index_map & (0x01UL << i))			/*set gpio 0~31 mode*/
            {
                ...
                {
                    if(deep_sleep_gpio_floating_map & (0x01UL << i)) 
                    {
...
//这个函数需要在 sctrl_enter_rtos_deep_sleep 函数前调用
