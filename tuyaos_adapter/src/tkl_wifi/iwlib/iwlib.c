 ...
 * terminated.
 */
void iw_essid_escape(char * dest,
        const char *    src,
        const int       slen)
{
            ...
            int     has_range)
{
    ...
    * because it does an explicit test on the flag. */
    ...
    else
    {
        ...
        snprintf(buffer, buflen,"Quality:%d  Signal level:%d  Noise level:%d",
            ...
            const struct timezone * tz)
{
    ...
    snprintf(buffer, buflen, "%02d:%02d:%02d.%06u", s / 3600,
        (s % 3600) / 60, s % 60, (u_int32_t) timev->tv_usec);
}
